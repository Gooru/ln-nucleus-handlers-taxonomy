...
class FetchStandardsFrameworkHandler implements DBHandler {
    ...
    public ExecutionResult<MessageResponse> executeRequest() {
        LazyList<AJEntityStandardFramework> results =
            AJEntityStandardFramework.findAll().orderBy(HelperConstants.SEQUENCE_ID);
        return new ExecutionResult<>(MessageResponseFactory.createOkayResponse(new JsonObject().put(STANDARD_FRAMEWORKS,
            new JsonArray(
                JsonFormatterBuilder.buildSimpleJsonFormatter(false, Arrays.asList(HelperConstants.TX_RESPONSE_FIELDS))
