...
class FetchCrosswalkGDFWCodesHandler implements DBHandler {
    ...
    public FetchCrosswalkGDFWCodesHandler(ProcessorContext context) {
    ...
    public ExecutionResult<MessageResponse> checkSanity() {
        ...
        if (context.request() == null || context.request().isEmpty()) {
            ...
            return new ExecutionResult<>(
                MessageResponseFactory.createInvalidRequestResponse("Invalid data provided to fetch crosswalk codes"),
                ExecutionResult.ExecutionStatus.FAILED);
        }
        ...
        if (this.codes == null || codes.isEmpty()) {
            ...
            return new ExecutionResult<>(
                MessageResponseFactory.createInvalidRequestResponse("input internal / display code missing"),
                ExecutionResult.ExecutionStatus.FAILED);
        }
    ...
    public ExecutionResult<MessageResponse> executeRequest() {
        ...
        final JsonObject mappedResults = new JsonObject();
        final String key = (this.isInternalCode == null || !this.isInternalCode) ? HelperConstants.SOURCE_DISPLAY_CODE
            : HelperConstants.SOURCE_TAXONOMY_CODE_ID;
        ...
        if (this.isInternalCode == null || !this.isInternalCode) {
            codes = AJEntityTaxonomyCodeMapping.where(AJEntityTaxonomyCodeMapping.DISPLAY_SOURCE_CODES_TO_TARGET_CODES,
                inputCodes, targetFrameworkId);
        } else {
            codes = AJEntityTaxonomyCodeMapping.where(AJEntityTaxonomyCodeMapping.INTERNAL_SOURCE_CODES_TO_TARGET_CODES,
                inputCodes, targetFrameworkId);
        }
        if (codes != null && codes.size() > 0) {
            codes.forEach(code -> {
                results.put(code.getString(key), new JsonObject(JsonFormatterBuilder
                    .buildSimpleJsonFormatter(false, Arrays.asList(HelperConstants.TX_CROSSWALK_CODES_RESPONSE_FIELDS))
                    .toJson(code)));
            });
        }
