...
public class TaxonomyTransformationHandler implements DBHandler {
    ...
    public ExecutionResult<MessageResponse> checkSanity() {
        ...
        if (context.request() == null || context.request().isEmpty()) {
            ...
            return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse(
                "invalid request received to do taxonomy transformation"), ExecutionResult.ExecutionStatus.FAILED);
        }
        ...
        if (standardPrefs == null || standardPrefs.isEmpty()) {
            ...
            return new ExecutionResult<>(
                MessageResponseFactory.createInvalidRequestResponse("user standard preference is missing"),
                ExecutionResult.ExecutionStatus.FAILED);
        }
        ...
        if (taxonomy == null || taxonomy.isEmpty()) {
            ...
            return new ExecutionResult<>(
                MessageResponseFactory.createInvalidRequestResponse("taxonomy is missing for transformation"),
                ExecutionResult.ExecutionStatus.FAILED);
        }
    ...
    private JsonObject transformTaxonomyCodeByUserPref() {
        ...
        if (targetCodes.size() > 0) {
            final List<AJEntityTaxonomyCodeMapping> sourceCodes =
                AJEntityTaxonomyCodeMapping.where(AJEntityTaxonomyCodeMapping.INTERNAL_TARGET_CODES_TO_SOURCE_CODES,
            ...
            if (sourceCodes.size() > 0) {
                ...
                sourceCodes.forEach(sourceCode -> {
                    ...
                    final String framework = standardPrefs.getString(subject);
                    codeMapping.put(sourceCodeId, sourceCode.getString(HelperConstants.TARGET_TAXONOMY_CODE_ID));
                ...
                frameworkCodeMap.forEach((framework, code) -> {
                    ...
                    if (userPrefCodes.size() > 0) {
                        userPrefCodes.forEach(codes -> {
                            ...
                            transformedCode.put(HelperConstants.CODE, codes.get(HelperConstants.TARGET_DISPLAY_CODE));
        ...
        taxonomy.fieldNames().forEach(id -> {
            ...
            standards.fieldNames().forEach(codeId -> {
                ...
                if (transformedCodes.containsKey(codeId)) {
                    ...
                    final String targetCodeId = transformedCode.getString(HelperConstants.TARGET_TAXONOMY_CODE_ID);
                    transformedCode.put(HelperConstants.PARENT_TITLE, code.getString(HelperConstants.PARENT_TITLE));
    ...
    private String getSubjectFromCodeId(String codeId) {
        return codeId.substring((codeId.indexOf(HelperConstants.DOT) + 1), codeId.indexOf(HelperConstants.HYPHEN));
    }
