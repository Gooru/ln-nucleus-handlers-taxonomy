...
class FetchStandardFrameworksHandler implements DBHandler {
  ...
  private static final Logger LOGGER = LoggerFactory
      .getLogger(FetchStandardFrameworksHandler.class);
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    LazyList<AJEntityFramework> results = AJEntityFramework.findAll()
    ...
    JsonArray jsonResults = new JsonArray(JsonFormatterBuilder
        .buildSimpleJsonFormatter(false,
            Arrays.asList(HelperConstants.TX_FRAMEWORK_RESPONSE_FIELDS)).toJson(results));
    return new ExecutionResult<>(
        MessageResponseFactory
            .createOkayResponse(new JsonObject().put(HelperConstants.FRAMEWORKS, jsonResults)),
