...
class FetchTaxonomySubtreeCodesHandler implements DBHandler {
    ...
    public ExecutionResult<MessageResponse> checkSanity() {
        ...
        if (context.standardFrameworkId() == null || context.standardFrameworkId().isEmpty() || context.codeId() == null
            || context.codeId().isEmpty()) {
    ...
    public ExecutionResult<MessageResponse> executeRequest() {
        LazyList<AJEntityCode> results =
            AJEntityCode.where(AJEntityCode.TAXONOMY_SUB_NODE_GET, context.codeId(), context.standardFrameworkId())
                .orderBy(HelperConstants.SEQUENCE_ID);
        return new ExecutionResult<>(MessageResponseFactory.createOkayResponse(new JsonObject().put(CODES,
            new JsonArray(JsonFormatterBuilder
                .buildSimpleJsonFormatter(false, Arrays.asList(HelperConstants.TX_DUMMY_CODES_RESPONSE_FIELDS))
